...
/**
 * Mark2 — Red Initial Autonomous
 *
 * Alliance:  RED
 * Start:     (122.05, 119.62) facing -143° — matches startPoint in Mark2RedInitialAuto.pp
 *
 * Path summary (derived from Mark2RedInitialAuto.pp)
 * ────────────────────────────────────────────────────
 *  1.  Drive to FireLocation                              → shoot preloads at red goal
 *  2.  Intake sweep — middle floor (IntakeFloorMiddle)    → intake on during drive
 *  3.  Return to FireLocation                             → shoot middle-floor pickup
 *  4–11. Four red gate-load cycles (FireLocation ↔ GateLoad) → intake on inbound,
 ...
 *
 * Tuning checklist:
 *   LAUNCH_DISTANCE_MAIN  — measure FireLocation → red goal (inches)
 *   LAUNCH_DISTANCE_FINAL — measure FinalFirePosition → red goal (inches)
 *   OUTTAKE_REVERSE_S     — inherited from Mark2AutoBase
 */
@Autonomous(name = "Mark2 Red Initial Auto", group = "Mark2")
public class Mark2RedInitialAuto extends Mark2AutoBase {

    // ── Starting pose — matches startPoint in Mark2RedInitialAuto.pp ──────────
    private static final double START_X   = 122.050;
    private static final double START_Y   = 119.623;
    private static final double START_ROT = -143.0;  // degrees — pp startPoint.startDeg

    // ── Launch distances (inches) — TUNE to actual measured red goal distances ─
    ...
    //  Source file  : Scripts\Mark2RedInitialAuto.pp
    //  Fragment file: Scripts\InputAndOutput\Mark2RedInitialAuto_points.javafragment
    //
    //  HOW TO UPDATE when paths change:
    //    1. Export updated paths from the Pedro visualizer to
    //         Scripts\Mark2RedInitialAuto.pp
    //       OR re-generate from Java using Convert-JavaToPedroPp.ps1.
    //    2. Run from the Scripts\ directory:
    //         .\Convert-PedroToJavaPoints.ps1 .\Mark2RedInitialAuto.pp
    //    3. Open Scripts\InputAndOutput\Mark2RedInitialAuto_points.javafragment
    ...
    protected void setup() {
        ...
        telemetry.addLine("Mark2 Red Initial Auto — Ready");
    ...
    protected void runPath() {
        ...
        //   Robot drives to FireLocation and shoots preloaded game elements at
        //   the red goal.
        ...
        // ── 4–11. Red gate-load cycles ─────────────────────────────────────────
        //   Four trips to the red GateLoad position to collect additional game
        //   elements.  Intake is active on every inbound (to-gate) leg.
