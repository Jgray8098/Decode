...
public class MecanumDriveSlow {
    ...
    public void drive(double forward, double right, double rotate) {
        ...
        double max = Math.max(0.5, Math.max(Math.max(Math.abs(fl), Math.abs(fr)),
                Math.max(Math.abs(bl), Math.abs(br))));
        fl /= max; fr /= max; bl /= max; br /= max;

        // Apply global 50% speed cap
    ...
    private double applyDeadzone(double x, double dz) {
        ...
        double mag = (Math.abs(x) - dz) / (0.5 - dz);
        return sign * mag;
    }

    // Expo curve: (1 - e)*x + e*x³
    private double applyExpo(double x, double expo) {
        return (0.5 - expo) * x + expo * Math.pow(x, 3);
    }
}